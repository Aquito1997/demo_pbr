...
const VAO_VBO &VertexManager::GetABPair(size_t index) { return vao_vbo[index]; }

void VertexManager::PushABPair(VAO_VBO abp) { vao_vbo.push_back(abp); }
...
size_t VertexManager::VertexNumber() const { return vao_vbo.size(); }
...
GLuint &VertexManager::VBO(size_t index) { return vao_vbo[index].VBO; }

size_t VertexManager::size() const { return vao_vbo.size(); }
...
RenderPipeline::RenderPipeline() { sp = 0; }
...
size_t RenderPipeline::GetTextureCount() const { return vecTex.size(); }
...
const GLuint &RenderPipeline::VAO(size_t index) { return this->vm.VAO(index); }
const GLuint &RenderPipeline::VBO(size_t index) { return this->vm.VBO(index); }
...
void RenderPipeline::Draw() { vm.Draw(); }

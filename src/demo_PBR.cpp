...
uint IBL::GenTexture::LoadHDRTexture(const std::string& picPath)
{
    return LoadHDRTexture(picPath.c_str());
}
uint IBL::GenTexture::LoadHDRTexture(const char* picPath)
{
    ...
    {
        std::cout << __FILE__ << ":" << __LINE__ << "  picPath: "
                  << picPath << "\t" << "Failed to load HDR image." << std::endl;
    }
    return hdrTexture;
}
...
void IBL::GenMap::GenCubeMap(fbAttri& fb)
{
    ...
    static const std::string hdrTexturePath = "./res/skybox/horn-koppe_snow_4k.hdr";
    assert(std::filesystem::exists(hdrTexturePath));
    GLuint hdrTexture = IBL::GenTexture::LoadHDRTexture(hdrTexturePath);
                           ...
                           const Devices& device)
{
    ...
    render.SetMat4("projection", Function::OpenGLBase::perspective(device, BaseTypeLen::halfAspect));
